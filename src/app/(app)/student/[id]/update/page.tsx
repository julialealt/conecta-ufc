...
export default function StudentProfileUpdatePage() {
  ...
  useEffect(() => {
    ...
    setEntryYear(userData?.entrySemester.split(".")[0] || "");
    setEntrySemester(userData?.entrySemester.split(".")[1] || "");
    setMonth(userData?.graduationForecast?.split(".")[1] || "");
    setYear(userData?.graduationForecast?.split(".")[0] || "");
    setIsLoading(false);
  }, [userData]);
  ...
  const handleUpdateProfile = async () => {
    try {
      const response = await localApi.put(`/students/${userId}/profile`, {
        name,
        description,
        about,
        course,
      ...
      if (response.status === 200) {
        const newStudentData: Student = response.data;
    ...
    <div className="self-stretch w-full px-30 pt-6 pb-16 bg-zinc-950 inline-flex flex-col justify-start items-start gap-8">
      <div className="w-full inline-flex justify-start items-start gap-3">
        ...
        <div className="w-full self-stretch inline-flex flex-col justify-center items-start gap-1">
          ...
          <div className="self-stretch justify-start text-zinc-300 text-sm font-medium leading-[150%]">
            Atualize suas informações e aumente suas oportunidades.
          </div>
        </div>
      ...
      <div className="self-stretch w-full inline-flex flex-col justify-start items-start gap-12">
        <div className="self-stretch inline-flex flex-col justify-start items-start gap-4">
          <div className="self-stretch justify-start text-white text-base font-medium leading-[150%]">
            Sobre
          </div>
          ...
          <div className="self-stretch inline-flex justify-start items-start gap-8">
            ...
            <div className="w-full self-stretch inline-flex justify-start items-end gap-2">
              <Select
                ...
                name="month"
                label={
                  graduated ? "Semestre de término*" : "Previsão de término*"
                }
                placeholder="Mês"
                value={month}
                options={[
